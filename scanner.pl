...
sub implies {
	...
	return
		...
		&& $opB =~ /^(?:>=?|<=?|==|eq|ne)$/;
	return
		unless defined( my $nA = coerce_number($vA) )
		&& defined( my $nB = coerce_number($vB) );

	# bail if either side doesn't look numeric
	return unless looks_like_number($vA) && looks_like_number($vB);
...
my @RULE_DEFS = (
	{
		...
		shortDescription =>
		  { text => "Redundant comparison in boolean expression" },
		helpUri => ''
	}, {
		id			  => "nested-threshold",
		...
		id			  => "always-true-test",
		shortDescription =>
		  { text => "Condition always true based on known constant" }
	}, {
		id			 => "always-false-test",
		shortDescription =>
		  { text => "Condition always false based on known constant" }
	},
);
	...
	for my $st (@$cmps) {
		...
		if(my($var, $op, $val)= parse_cond($raw)){
			if(exists $CONST{$var}){
				...
				# always-true?
				if($op =~ />=?/ && looks_like_number($c) && looks_like_number($val) && $c >= $val) {
					...
					next;	# skip further heuristics on this stmt
				}

				# always-false?
				elsif($op =~ /<=?/ && looks_like_number($c) && looks_like_number($val) && $c <= $val){
...
qq{"$comps[$i]" redundant in $bool_op with "$comps[$j]"};
		...
		for my $st (@$cmps) {
			...
			my($v0, $o0, $x0)= parse_cond( $chain->[0]->schild(1)->content )
			  or next;
			for my $i(1 .. $#$chain){
				my $el = $chain->[$i];
				...
				if(implies( $o0, $x0, $o1, $x1)) {
					printf "[%s:%d] redundant elsif “%s” implied by “%s”\n",
					  $file, $el->line_number, $raw,
					  $chain->[0]->schild(1)->content;
				}
			}
		}
	}
	...
	#
	{
		...
		# 1) bucket each block by its trimmed text
		for my $blk (@$blocks) {
			...
			$t =~ s/^\s+|\s+$//g;
			my $nlines = ($t =~ tr/\n//) + 1;
			next if $nlines <= 2;  # skip tiny blocks
			next if length($t) < 40;	# skip short blocks
			push @{ $buckets{$t} }, $blk->line_number;
		}

		# 2) emit one warning per bucket with >1 occurrence
